...
--		 EXECUTE [dbo].[PG_LI_HEADER_RMA] 0,56,'20200910',-1,'( TODOS )',null,null
--		 EXECUTE [dbo].[PG_LI_HEADER_RMA] 0,139,'20200910',-1,'( TODOS )',null,null
--		 EXECUTE [dbo].[PG_LI_HEADER_RMA] 0,167,'20200910',-1,'( TODOS )',null,null
--		 EXECUTE [dbo].[PG_LI_HEADER_RMA] 0,167,'',-1,'( TODOS )',null,null

--		 EXECUTE [dbo].[PG_LI_HEADER_RMA] 0,87,'',-1,'( TODOS )',null,null
--		 EXECUTE [dbo].[PG_LI_HEADER_RMA] 0,165,'',-1,'( TODOS )',null,null
--		 EXECUTE [dbo].[PG_LI_HEADER_RMA] 0,139,'',-1,'( TODOS )',null,null
--		 EXECUTE [dbo].[PG_LI_HEADER_RMA] 0,144,'',-1,'( TODOS )',null,null
	...
	--BEGIN
		...
		--BEGIN				
				...
				AND			USUARIO_PEARL.K_USUARIO_DEPARTAMENTO	=	(	CASE
																				WHEN	@PP_K_USUARIO_ACCION	IN (43,87,98,139,144) THEN	USUARIO_PEARL.K_USUARIO_DEPARTAMENTO
...
BEGIN TRY
	...
	--============================================================================
		...
		VALUES	
			...
			IF @@ROWCOUNT = 0
			BEGIN
				SET @VP_MENSAJE='No se generó. [HDR#'+CONVERT(VARCHAR(10),@VP_K_HEADER_RMA)+']'
...
BEGIN TRY
	...
	IF @@ROWCOUNT = 0
	BEGIN
		SET @VP_MENSAJE='Registro no fue modificado. [HDR#'+CONVERT(VARCHAR(10),@PP_K_HEADER_RMA)+']'
		RAISERROR (@VP_MENSAJE, 16, 1 ) 
	END		
		...
		BEGIN
			...
			SELECT @VP_VALOR_PRECIOM	= LEFT(@PP_ARRAY_PRECIOM	, @VP_POSICION_PRECIOM	- 1)
				...
				AND		LTRIM(RTRIM(ITEMNO))	= @VP_VALOR_ITEM_NO
					...
					IF @PP_K_TIPO_RMA	IN	(2)		-- SON LAS ORDENES DE TIPO CORTE ESPECIAL
					BEGIN
						SET	@VP_VALOR_CLAVE_D	= 'XSR'
						SET	@VP_VALOR_DEFECTO	= 'ORDEN DE SERVICIO'
					END
					ELSE							-- SI NO ES ORDEN ESPECIAL ES UNA ORDEN DE REEMPLAZO, POR ESO EL PRECIO SE COLOCA EN COSTO 0, NINGUNA RMA SE COBRA.
					BEGIN	
...
--		 EXECUTE [dbo].[PG_UP_ESTATUS_RMA]	0,56,  '62/63',2				---CANCELAR
--		 EXECUTE [dbo].[PG_UP_ESTATUS_RMA]	0,139,  44,2
--		 EXECUTE [dbo].[PG_UP_ESTATUS_RMA]	0,139,  46,2
...
BEGIN TRY
		-- ===========================
DECLARE  @VP_POSICION_K_HEADER		INT
...
-- /////////////////////////////////////////////////////////////////////
	...
	WHILE patindex('%/%' , @PP_ARRAY_K_HEADER) <> 0
		BEGIN
			...
			IF @@ROWCOUNT = 0
			BEGIN
				SET @VP_MENSAJE='No se encontró el estatus de la [Orden#'+CONVERT(VARCHAR(10),@VP_VALOR_K_HEADER)+']...'
				RAISERROR (@VP_MENSAJE, 16, 1 ) 
			END			
			...
			-- //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
				...
				BEGIN 					
					...
					END
						...
						IF @@ROWCOUNT = 0
						BEGIN
							SET @VP_MENSAJE='La [Orden#'+CONVERT(VARCHAR(10),@VP_VALOR_K_HEADER)+'] no fue encontrada...'
							RAISERROR (@VP_MENSAJE, 16, 1 ) 
						END			
				END
			-- /////////////////////////////////////////////////////////////////////
				...
				BEGIN 
					...
					--END
						...
						IF @PP_K_USUARIO_ACCION IN (56) AND @VP_STATUS_K_HEADER IN (2)				-- MANUELG
						BEGIN
						...
						END
							...
							IF @@ROWCOUNT = 0
							BEGIN
								SET @VP_MENSAJE='La [Orden#'+CONVERT(VARCHAR(10),@VP_VALOR_K_HEADER)+'] no fue actualizada...'
								RAISERROR (@VP_MENSAJE, 16, 1 ) 
							END
			...
			-- /////////////////////////////////////////////////////////////////////
				...
				BEGIN
					...
					BEGIN
						...
						RAISERROR (@VP_MENSAJE, 16, 1 ) 
					END
					ELSE IF @VP_STATUS_K_HEADER >= 6 AND ( @PP_K_USUARIO_ACCION	<> 139)	-- YA NO SE PUEDE CANCELAR, YA ESTA SIENDO PROCESADA.
					BEGIN
...
GO
--		 EXECUTE [dbo].[PG_UP_ESTATUS_ORDEN_IMPRESA_RMA]	0,139,52235
...
BEGIN TRY
	--/////////////////////////////////////////////////////////////
		...
		IF @@ROWCOUNT = 0
		BEGIN
			SET @VP_MENSAJE='No se obtuvo el estatus de la [Orden#'+CONVERT(VARCHAR(10),@PP_K_HEADER_RMA)+'], verifique...'
			RAISERROR (@VP_MENSAJE, 16, 1 ) 
		END			
