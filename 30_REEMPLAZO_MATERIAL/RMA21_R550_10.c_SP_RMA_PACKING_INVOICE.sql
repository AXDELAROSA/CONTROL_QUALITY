...
GO
--		 EXECUTE [dbo].[PG_LI_HEADER_RMA_PACKING] 0,139, 'IRVI02' , 30 , 1 
--		 EXECUTE [dbo].[PG_LI_HEADER_RMA_PACKING] 0,139, '( TODOS )' , 30 , 2
...
GO
--		 EXECUTE [dbo].[PG_LI_DETAILS_RMA_PACKING] 0,139, '41198'
...
AS
	...
	BEGIN
		SELECT	K_TIPO_RMA,
				...
				--	) AS KIT,
				--DETAILS_RMA.CANTIDAD_ORDENADA			AS CANTIDAD_ENVIADA,	-- SE QUITA LA CANTIDAD DEL DETALLE DEL RMA, PARA PODER REALIZAR MODIFICACIONES EN LA TABLA INVENTARIO EMBARQUE.
				INVENTARIO_EMBARQUE_RMA.QTY				AS CANTIDAD_ENVIADA,
...
GO
--		 EXECUTE [dbo].[PG_GET_DETALLE_PACKING_RMA] 0 ,0,  '1590' , 'XXXXXXXXXX'
...
AS
	...
	BEGIN
		...
		SELECT	K_HEADER_RMA
		FROM	DETAILS_RMA		(NOLOCK)
		WHERE	K_DETAILS_RMA	IN	( SELECT DISTINCT K_DETAILS_RMA FROM @TBL_MATERIAL_SELECCIONADO )--= @VP_VALOR_K_DETAILS_RMA
		SET NOCOUNT ON
	END
	--===============================================================================================================================================================
	--===============================================================================================================================================================
...
--		 EXECUTE [DBO].[PG_LI_HEADER_RMA_INVOICE] 0,0, '( TODOS )',1
...
AS
	...
	SELECT	DISTINCT	
			...
			CUSTOMER
	FROM	INVENTARIO_EMBARQUE_RMA				(NOLOCK)
	WHERE	K_ESTATUS_INVENTARIO_EMBARQUE_RMA	IN ( 3 )	--,4 ) LAS FACTURADAS SE PUEDEN OBSERVAR EN LA PANTALLA DE REEEMPLAZOS.
	AND		( CUSTOMER	= @PP_CUSTOMER	OR	@PP_CUSTOMER = '( TODOS )' )
	AND		SERIAL_2	IN  (
...
GO
--		 EXECUTE [DBO].[PG_LI_DETAILS_RMA_BAJA] 0,0, '2203090001', 'FAUR01'
...
AS
	...
	SELECT	K_TIPO_RMA,
			...
			'-'								AS INVOICE_NO,
			--D_STATUS_RMA,
			CODIGO_ETIQUETA					AS D_STATUS_RMA,
			[DETAILS_RMA].*
	FROM	[DETAILS_RMA]				(NOLOCK)
	INNER JOIN	IMITMIDX_SQL			(NOLOCK)	ON IMITMIDX_SQL.ITEM_NO		= DETAILS_RMA.ITEM_NO
	INNER JOIN	STATUS_RMA				(NOLOCK)	ON STATUS_RMA.K_STATUS_RMA	= DETAILS_RMA.K_STATUS_RMA	
	...
	INNER JOIN	HEADER_RMA				(NOLOCK)	ON HEADER_RMA.K_HEADER_RMA	= DETAILS_RMA.K_HEADER_RMA
	INNER JOIN	INVENTARIO_EMBARQUE_RMA	(NOLOCK)	ON INVENTARIO_EMBARQUE_RMA.SERIAL_2	= DETAILS_RMA.K_DETAILS_RMA
	INNER JOIN	MATERIAL_PROGRAMADO		(NOLOCK)	ON MATERIAL_PROGRAMADO.SERIAL		= INVENTARIO_EMBARQUE_RMA.SERIAL_1
	WHERE	DETAILS_RMA.K_DETAILS_RMA	IN	(	SELECT	SERIAL_2
												FROM	INVENTARIO_EMBARQUE_RMA	(NOLOCK)
...
AS
	...
	BEGIN			
			...
			INNER JOIN	BD_GENERAL.dbo.ARCUSFIL_TERMS_PERIOD			(NOLOCK) ON ARCUSFIL_SQL.K_ARCUSFIL_TERMS_PERIOD	= ARCUSFIL_TERMS_PERIOD.K_ARCUSFIL_TERMS_PERIOD
...
AS
	...
	BEGIN
			...
			BEGIN
					...
					ELSE
					BEGIN
						--	VERIFICAMOS SI LA ORDEN ESPECIAL DE CORTE APLICA COBRO
						SELECT	@VP_APLICA_COBRO	 = L_APLICA_COBRO
						FROM	HEADER_RMA		(NOLOCK)
						WHERE	K_HEADER_RMA	IN (	SELECT TOP (1) K_HEADER_RMA
														FROM	[DETAILS_RMA]		(NOLOCK)
														WHERE	K_DETAILS_RMA		=	@VP_CU_SERIAL_2	)

						IF @VP_APLICA_COBRO	= 0
						BEGIN
							SET	@VP_TOTAL_KIT			= 0
						END
					END								
		...
		BEGIN
			...
			WHILE @@FETCH_STATUS = 0
				BEGIN
					...
					END						
					-- ///////////////////////////////////////////
					--	SE REALIZA EL CALCILO DEL TOTAL
					DECLARE @VP_TOTAL_KIT DECIMAL(13,2) = 0
...
--	--------------------------------------------------------------------------------------------------------------------------------------------
--		 EXECUTE [DBO].[PG_UP_INVENTARIO_EMBARQUE_PACKING_RMA] 1,87,  'JJT' , '110/126/127/107/108/109/106/114/112/113/111/115/121/120/118/119/124/123/122/185/116/117/125/182/183/184/100/101/102/103/104/105/128' , 2 , 1 , 1 
				...
				BEGIN
					...
					IF @VP_N_PACKING_X_PROGRAMA > 0
						BEGIN
							...
							AND		CONVERT(DATE, F_INVENTARIO_EMBARQUE_LOG_RMA)	= @VP_DATE
							AND		LTRIM(RTRIM(PACKING_NO))						LIKE CONCAT(@VP_PROGRAMA, '%')
							ORDER	BY	F_INVENTARIO_EMBARQUE_LOG_RMA DESC
							...
							DECLARE  @VP_DELIMITADOR				VARCHAR(5)	= '-'
									...
									,@VP_POSICION_GUION_PRIMERO		INT			= 0
									,@VP_POSICION_GUION_SEGUNDO		INT			= 0
									,@VP_CONSECUTIVO_NUEVO			INT			= 0

							--SET @VP_POSICION_GUION = CHARINDEX(@VP_DELIMITADOR, @VP_PACKING_NO_ACTUAL)
							SET @VP_POSICION_GUION_PRIMERO = CHARINDEX(@VP_DELIMITADOR, @VP_PACKING_NO_ACTUAL)
							--SET @VP_CONSECUTIVO_ACTUAL = SUBSTRING(@VP_PACKING_NO_ACTUAL, @VP_POSICION_GUION + 1, 5)
							SET @VP_INFO_UN_GUION		= SUBSTRING(@VP_PACKING_NO_ACTUAL, @VP_POSICION_GUION_PRIMERO+1, 25)
							--SET @VP_CONSECUTIVO_NUEVO = CONVERT(INT, @VP_CONSECUTIVO_ACTUAL) + 1
							SET @VP_POSICION_GUION_SEGUNDO	= CHARINDEX(@VP_DELIMITADOR, @VP_INFO_UN_GUION)
...
AS
	...
	SET @VP_PACKING_NUEVO = CONCAT(FORMAT(GETDATE(),'yyMMdd'), FORMAT(@VP_N_PACKING,'0000') )
