...
AS
	...
	-- ///////////////////////////////////////////
	 ...
	 SELECT	(CASE WHEN (SELECT COUNT(K_INSPECCION_MATERIAL) FROM [INSPECCION_MATERIAL] WHERE NUMERO_PARTE = LTRIM(RTRIM(ITEM_NO))) > 0 
					THEN 1
...
GO
/*
 EXEC	[dbo].[PG_SK_CONFIGURAR_MATERIAL_INSPECCIONAR] 0,144, 200435AWT3
*/
...
AS
	...
	IF @VP_PORCENTAJE_APROBATORIO IS NULL
	 ...
	 SELECT	0							AS PROGRAMADO,
			LTRIM(RTRIM(ITEM_NO))		AS NUMERO_PARTE,
			@VP_PORCENTAJE_APROBATORIO	AS PORCENTAJE_APROBATORIO,
			LTRIM(RTRIM(ITEM_DESC_1))	AS DESCRIPCION,
			LTRIM(RTRIM(PROD_CAT))		AS PRODUCTO_CATEGORIA,
			LTRIM(RTRIM(LOC))			AS LOCACION
	 FROM	IMITMIDX_SQL 
