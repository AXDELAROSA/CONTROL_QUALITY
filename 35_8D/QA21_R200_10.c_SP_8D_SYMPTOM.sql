...
AS
	...
	-- ///////////////////////////////////////////
	SELECT	[8D_SYMPTOM_ACTION].[K_8D_SYMPTOM_ACTION],	
			-- =================
			[K_8D],				
			-- =================								
			[ACCION],			
			[PORCENTAJE],			
			D_USUARIO_PEARL,
			CONVERT(DATE, [8D_SYMPTOM_ACTION].[F_CAMBIO]) AS [DATE],
			-- =================
			ISNULL((	SELECT TOP 1 (CASE WHEN [8D_SYMPTOM_EVIDENCE].K_8D_SYMPTOM_ACTION IS NULL THEN 'NO'
								  	ELSE 'YES' END)
				FROM [8D_SYMPTOM_EVIDENCE] (NOLOCK) 
				WHERE K_8D_SYMPTOM_ACTION = [8D_SYMPTOM_ACTION].K_8D_SYMPTOM_ACTION ), 'NO') AS EVIDENCIA
			-- =================
...
/*
 EXEC	[dbo].[PG_SK_8D_SYMPTOM_EVIDENCE] 0,144,2
*/
...
AS
	...
	SELECT	[K_8D_SYMPTOM_EVIDENCE],
			...
			-- =================					
			D_USUARIO_PEARL,
			CONVERT(DATE, [8D_SYMPTOM_EVIDENCE].[F_CAMBIO]) AS [DATE]		
			-- =================
	FROM	[8D_SYMPTOM_EVIDENCE] (NOLOCK)
	...
	IF @VP_MENSAJE=''
		BEGIN
			...
			BEGIN TRY
				...
				ELSE
					BEGIN
						UPDATE [8D_SYMPTOM_ACTION]
							SET [ACCION] = @PP_ACCION,
								[PORCENTAJE] = @PP_PORCENTAJE,
								[K_USUARIO_CAMBIO] = @PP_K_USUARIO_ACCION,
								[F_CAMBIO] = GETDATE()
						WHERE [K_8D] = @PP_K_8D
						AND [K_8D_SYMPTOM_ACTION] = @PP_K_SYMPTOM_ACTION

						IF @@ROWCOUNT = 0
							RAISERROR ('ERROR: No fue posible Actualizar la acción del sintoma para la [8D].', 16, 1 ) --MENSAJE - Severity -State.
					END
...
GO
/*
 EXECUTE [PG_IN_SYMPTOM_EVIDENCE] 0,144, 3 , 2 , 'CAMBIO # PARTE.xlsx' , '.xlsx' 
*/
...
AS			
	...
	IF @VP_MENSAJE=''
		BEGIN
			...
			BEGIN TRY
				...
				DECLARE @CREAR_CARPETA NVARCHAR(255) = N'\\10.1.1.5\documents\Quality\8D POR SISTEMA\' + CONVERT(VARCHAR(25), @PP_K_8D)+ '\SYMPTOM'
				SET NOCOUNT ON
				...
				IF @@ROWCOUNT = 0
					RAISERROR ('ERROR: No fue posible agregar la evidencia del sintoma para la [8D].', 16, 1 ) --MENSAJE - Severity -State.
	...
	IF @VP_MENSAJE<>''
		BEGIN
			SET		@VP_MENSAJE = 'No es posible [Agregar] la Evidencia del sintoma para la [8D]: ' + @VP_MENSAJE 
			SET		@VP_MENSAJE = @VP_MENSAJE + ' ( '
			SET		@VP_MENSAJE = @VP_MENSAJE + '[#8D.'+CONVERT(VARCHAR(10),@PP_K_8D)+']'
			SET		@VP_MENSAJE = @VP_MENSAJE + ' )'
		END
...
AS
	...
	IF @VP_MENSAJE<>''
		BEGIN
			...
			SET		@VP_MENSAJE = @VP_MENSAJE + ' ( '
			SET		@VP_MENSAJE = @VP_MENSAJE + '[#Evid.'+CONVERT(VARCHAR(10),@PP_K_8D_SYMPTOM_EVIDENCE)+']'
			SET		@VP_MENSAJE = @VP_MENSAJE + ' )'
		END
