...
USE PPMS_PEARL
	...
	SELECT * FROM [PPMS_PEARL].[dbo].personal (NOLOCK) WHERE INSPECTOR_CAL = 'SANTIAGO SANCHEZ ZETINA                           '
	SELECT * FROM [PPMS_PEARL].[dbo].personal (NOLOCK) WHERE SELLO = '386'
	...
	SELECT TOP 1000	* FROM [PPMS_PEARL].[dbo ].certificacion_rpt (NOLOCK) --WHERE INSP_CERTI = 'SANTIAGO SANCHEZ ZETINA' --ESTACION = 'QCCERT-010' --COSTO > 0 --AND CONVERT(DATE,FECHA) = '2021-12-14'
	ORDER BY ID DESC
